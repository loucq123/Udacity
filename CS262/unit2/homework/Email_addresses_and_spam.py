...
def t_EMAIL(token):
	...
	token.value = re.sub('NOSPAM', '', token.value)    # Replace the 'NOSPAM' in token.value with ''
	return token
...
NOSPAMuser@uucpgateway.somedomain.example.com test tester testing"""

output1 = ['louiseaston@germany.de', 'irmgardkeun@weimar.de',
		   'rahelvarnhagen@berlin.de', 'user@uucpgateway.somedomain.example.com']
